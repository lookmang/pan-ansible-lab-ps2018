...
def download_software(dvsn,panorama_ip,api_key):
    try:
        cmd="<request><system><software><download><version>8.0.3</version></download></software></system></request>"
        parameters = {'target':dvsn}
        device = base.PanDevice.create_from_device(panorama_ip, api_key=api_key)
        xmldoc=device.op(cmd=cmd,cmd_xml=False,extra_qs=parameters)
...
def check_software(dvsn,panorama_ip,api_key):
    try:
...
def install_software(dvsn,panorama_ip,api_key):
    try:
        cmd="<request><system><software><install><version>8.0.3</version></install></software></system></request>"
        parameters = {'target':dvsn}
        device = base.PanDevice.create_from_device(panorama_ip, api_key=api_key)
        xmldoc=device.op(cmd=cmd,cmd_xml=False,extra_qs=parameters)
        jobid=xmldoc.find('./result/job')
...
def restart_fw(dvsn,panorama_ip,api_key):
    try:
...
def main():
    ...
    time.sleep(60*2)
    #Check AV Contents
    chk_status=check_software(dvsn,ip_address,api_key)

    time.sleep(60)
    #Download AV Contents
    jobid= download_software(dvsn,ip_address,api_key)
    if jobid["retval"]!="Error":
        ...
        if job_status=="FIN":
            ...
            #Install AV Contents
            install_jobid=install_software(dvsn,ip_address,api_key)
            ...
            if install_jobid["retval"]!="Error":
                ...
                if install_status=="FIN":
                    restart_fw(dvsn,ip_address,api_key)
                    response = {"retval": "Software installed successfully"}
